...
if [ $stage -le 0 ]; then
    ...
    echo --------------------- local/wsj_prepare_dict.sh
	...
	(
	    # import librispeech LM. DOESN'T WORK ATM
	    echo --------------------- import LM
	    local/wsj_extend_dict.sh --dict-suffix "_nosp" $wsj1/13-32.1  && \
	    utils/prepare_lang.sh data/local/dict_nosp_larger \
	        "<SPOKEN_NOISE>" data/local/lang_tmp_nosp_larger data/lang_nosp_bd && \
            local/download_lm.sh $lm_url  data/local/librispeech_lm
	) &     
    fi
...
if [ $stage -le 2 ]; then
  ...
  if $decode; then
    ...
    for data in eval92; do #removed dev93
