...
for f in data/${all_train}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
...
if [ $stage -le 2 ] && [ -f data/${all_train}_sp_hires/feats.scp ]; then
...
if [ $stage -le 2 ]; then
  ...
  for train_set in $train_sets; do
      ...
      steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
			 --cmd "$train_cmd" data/${train_set}_sp_hires
...
if [ $stage -le 3 ]; then
  ...
  num_utts_total=$(wc -l <data/${all_train}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh data/${all_train}_sp_hires \
      $num_utts ${temp_data_root}/${all_train}_sp_hires_subset

  echo "$0: computing a PCA transform from the hires data."
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      --splice-opts "--left-context=3 --right-context=3" \
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${all_train}_sp_hires_subset \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    ${temp_data_root}/${all_train}_sp_hires_subset 512 \
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/${all_train}_sp_hires exp/nnet3${nnet3_affix}/diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 5 ]; then
  ...
  ivectordir=exp/nnet3${nnet3_affix}/ivectors_${all_train}_sp_hires
...
if [ -f data/${all_train}_sp/feats.scp ] && [ $stage -le 8 ]; then
...
if [ $stage -le 8 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${all_train}_sp data/$train_lang $gmm_dir $ali_dir
fi
