...
srcdir=$1
mkdir -p data/local/wsj_dict_larger
dir=data/local/wsj_dict_larger
cp -r data/local/wsj_dict/* \
  data/local/wsj_dict_larger # Various files describing phones etc.
                                       # are there; we just want to copy them
                                       # as the phoneset is the same.
rm data/local/wsj_dict${dict_suffix}_larger/lexicon.txt  # we don't want this.
rm data/local/wsj_dict${dict_suffix}_larger/lexiconp.txt # we don't want this either.
mincount=2 # Minimum count of an OOV we will try to generate a pron for.

[ ! -f data/local/wsj_dict${dict_suffix}/cmudict/cmudict.0.7a ] && \
...
grep -v ';;;' data/local/wsj_dict${dict_suffix}/cmudict/cmudict.0.7a |
