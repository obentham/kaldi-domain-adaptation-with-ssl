...
# case insensitive
local/swbd1/swbd1_map_words.pl -f 2- $dir/transcripts2.txt  > $dir/text
...
awk -v sph2pipe=$sph2pipe '{
  printf("%s-A %s -f wav -p -c 1 %s | sox -b 16 -r 8k -t wav - -b 16 -r 16k -t wav - |\n", $1, sph2pipe, $2); 
  printf("%s-B %s -f wav -p -c 2 %s | sox -b 16 -r 8k -t wav - -b 16 -r 16k -t wav - |\n", $1, sph2pipe, $2);
...
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel; do
  cp $dir/$f $final_dir/$f || exit 1;
done

if [ $# == 2 ]; then # fix speaker IDs
  find $2 -name conv.tab > $dir/conv.tab
  local/swbd1/swbd1_fix_speakerid.pl `cat $dir/conv.tab` $final_dir
  utils/utt2spk_to_spk2utt.pl $final_dir/utt2spk.new > $final_dir/spk2utt.new
  # patch files
  for f in spk2utt utt2spk text segments; do
    cp $final_dir/$f $final_dir/$f.old || exit 1;
    cp $final_dir/$f.new $final_dir/$f || exit 1;
  done
  rm $dir/conv.tab
fi 
...
utils/fix_data_dir.sh $final_dir
