...
| name        | code | domain  | description       | data           | hours |
|-------------|------|---------|-------------------|----------------|------:|
| WSJ         | wsj  | general | news broadcast    | text and audio | 82    |
| Switchboard | swbd | target  | spontaneous phone | audio          | 260   |
| Fisher      | fish | target  | spontaneous phone | text           | N/A   |
...
1. Create 2 LMs, one trained on wsj (lm_wsj), and one trained on fish (lm_fish).

2. Run Kaldi to generate baseline alignments prior to starting Pytorch-Kaldi, trained with lm_wsj.

3. For each LM (only lm_wsj now), train a Pytorch-Kaldi system on wsj for each split (described below). Although only training on a single LM, decode both wsj and swbd with both LMs to generate the solid lines in the graph above.
...
For now, I'm training baseline model on wsj using lm_wsj, might look into doing it with lm_fish later. Along with split{1, 2, 3} that will be used for SSL, I need to train a model on many splits of wsj, since the x-axis of the curve above is amount of wsj train data. The wsj subsets that I'm going with for now are:
